<aside class="w-64 bg-foam border-r-3 border-ink flex flex-col justify-between min-h-screen hidden md:flex">
    ...
    <nav class="flex-1 overflow-y-auto py-6">
        <ul class="space-y-3 px-4">
            ...
            <li><a href="#" class="block px-4 py-3 font-bold text-ink border-3 border-transparent hover:border-ink hover:shadow-brutal hover:bg-ocean hover:text-white transition-all">🛒 Kelola Pesanan</a></li>
            <li><a href="#" class="block px-4 py-3 font-bold text-ink border-3 border-transparent hover:border-ink hover:shadow-brutal hover:bg-ocean hover:text-white transition-all">📈 Laporan Penjualan</a></li>
        </ul>
    </nav>
    <div class="border-t-3 border-ink p-4 bg-white">
        <div class="font-bold text-sm mb-1 truncate">{{ Auth::user()->name }}</div>
        <div class="text-xs text-ocean mb-4 font-bold uppercase tracking-wider">Sales Manager</div>
